...
public class AnnotatedClass {
     ...
     * 长按方法
     */
    public List<OnLongClickMethod> mLongClickMethod;
    ...
    public AnnotatedClass(TypeElement classElement, Elements elementUtils) {
        ...
        this.mLongClickMethod = new ArrayList<>();
    }
    ...
    public void addLongClickMethod(OnLongClickMethod method){
        mLongClickMethod.add(method);
    }
    ...
    public JavaFile generateFinder() {
         ...
         * 声明longClickListener
         */
        if(mLongClickMethod.size() > 0){
            methodBuilder.addStatement("$T longClickListener", TypeUtil.ONLONGCLICK_LISTENER);
        }

        for(OnLongClickMethod method : mLongClickMethod){
            TypeSpec longClickListener = TypeSpec.anonymousClassBuilder("")
                    .addSuperinterface(TypeUtil.ONLONGCLICK_LISTENER)
                    .addMethod(MethodSpec.methodBuilder("onLongClick")
                            .addAnnotation(Override.class)
                            .addModifiers(Modifier.PUBLIC)
                            .returns(TypeName.BOOLEAN)
                            .addParameter(TypeUtil.ANDROID_VIEW, "view")
                            .addStatement("return host.$N()", method.getMethodName())
                            .build())
                    .build();
